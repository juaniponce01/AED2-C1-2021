...
TEST(Ej, 07) {
  ...
  EXPECT_EQ(interseccion(set<int>({1, 2, 3}), set<int>({2, 1, 4, 5, 3})), set<int>({1, 2, 3}));
}
...
TEST(Ej, 08) {
  ...
  map<int, set<int>> r6 = agrupar_por_unidades(vector<int>({1, 2, 3, 12, 20, 14}));
  map<int, set<int>> e6 = map<int, set<int>>({{1, set<int>{1}}, {2, set<int>{2, 12}}, {0, set<int>{20}},
                                              {3, set<int>{3}}, {4, set<int>{14}}});
  EXPECT_EQ(r6, e6);
}
